...
cd /opt/lugrouplib/server
python3 -m venv /opt/lugrouplib/venv
/opt/lugrouplib/venv/bin/pip install -r requirements.txt

# Smoke test (Ctrl-C to stop):
/opt/lugrouplib/venv/bin/python app.py
```
...
Everything lives under `/opt/lugrouplib/library/` (the SQLite DB and all KiCad files). Back up
that one folder and you have backed up the whole library.

## Updating

```bash
cd /opt/lugrouplib && git pull
