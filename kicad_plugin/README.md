...
1. Build the package (or use the prebuilt one in `dist/`):
   ```bash
   python kicad_plugin/build_pcm_package.py     # -> dist/HackLib-Sync-1.0.0.zip
   ```
2. KiCad main window → **Plugins → Plugin and Content Manager**.
3. Click **Install from File…** and pick `dist/HackLib-Sync-1.0.0.zip`.
4. Apply/close. Open the **PCB Editor**; a green ⤓ **HackLib: Sync Library** button is on
   the toolbar.
...
2. Copy the **`hacklib_sync`** folder (the one with `__init__.py`) into that directory.
...
- **Easiest:** the first click asks for the server URL (`http://YOUR_VPS_IP:8000`) and
  remembers it. The target folder comes automatically from the `HACKLIB_DIR` environment
  variable you set in KiCad (Preferences → Configure Paths), else `~/Documents/KiCad_HackLib`.
- **Pre-configure:** copy `hacklib_config.example.json` → `hacklib_config.json` next to the
  plugin and set the values. Leave `local_dir` as `""` to use `HACKLIB_DIR`.

## Use

Click **HackLib: Sync Library** → it downloads and extracts the bundle, then shows how
...
```
client/
  hacklib_core.py               # CANONICAL shared sync core (download + extract)
  sync_client.py                # CLI wrapper over hacklib_core
kicad_plugin/
  hacklib_sync/                 # the plugin (UI + registration)
    __init__.py                 #   ← wx UI + ActionPlugin, imports hacklib_core
    hacklib_core.py             #   ← copy of client/hacklib_core.py (build keeps it in sync)
    icon.png                    #   24x24 toolbar icon
    hacklib_config.example.json
  pcm/
    metadata.json               # PCM manifest (v1 schema, KiCad 6.0+)
    resources/icon.png          # 64x64 package icon shown in the PCM
  build_pcm_package.py          # copies the core + zips into dist/HackLib-Sync-<v>.zip
  dist/HackLib-Sync-1.0.0.zip   # ready to "Install from File"
```

**One implementation, shared:** the download/extract code lives once in
`client/hacklib_core.py`. The CLI imports it directly; `build_pcm_package.py` copies it
